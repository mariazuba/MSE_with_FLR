...
stk.om.im.err <- fwd(stk, control=ctrl, sr=srbh, sr.residuals = exp(srbh.res), sr.residuals.mult = TRUE)
...
im.err <- FLQuant(runif(prod(dim(stk.om.im.err@landings)),1.1,1.2), dimnames = dimnames(stk.om.im.err@landings))
...
for(i in vy[-length(vy)]){
  ...
  ctrl   <- getCtrl(c(TAC[,ac(ay+1)]*im.err[,ac(ay+1)]), "catch", ay+1, it)
  stk.om.im.err <- fwd(stk.om.im.err, control=ctrl,sr=srbh, sr.residuals = exp(srbh.res), sr.residuals.mult = TRUE) 
}
...
stk.tmp <- window(stk.om.im.err,start=iy)
...
plot(FLStocks(stk.om.im.err=stk.om.im.err, stk.mp=stk.mp)) + theme(legend.position="top") + geom_vline(aes(xintercept=2009))


save(stk.om.im.err, file = 'C:/use/Dropbox/FLBEIA Course/Tutorials/02_MSE_with_FLR/stk_exe5.RData')
#---------------------------------------------------------------------------------
# EXERCISES
#---------------------------------------------------------------------------------
