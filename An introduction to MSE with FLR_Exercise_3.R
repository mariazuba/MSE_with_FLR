...
o <- function(stk, idx, assessmentYear, dataYears) {
  ...
  # Change the observation of M in the OM
  stk.tmp@m[] <- 0.1
...
# Change the m
stk.om.merr     <- stk.om
stk.om.merr@m[] <- runif(prod(dim(stk.om@m)),0.05,0.2)
#---------------------------------------------------------------------------------
# Start the MSE loop
#---------------------------------------------------------------------------------
...
for(i in vy[-length(vy)]){
  ...
  stk.om.merr <- fwd(stk.om.merr, control=ctrl,sr=srbh, sr.residuals = exp(srbh.res), sr.residuals.mult = TRUE) 
}

save(stk.om.merr, file = 'C:/use/Dropbox/FLBEIA Course/Tutorials/02_MSE_with_FLR/stk_exe3.RData')

#---------------------------------------------------------------------------------
# PERFORMANCE STATISTICS
#---------------------------------------------------------------------------------
# Some example performance statstics, but first isolate the projection period
stk.tmp <- window(stk.om.merr,start=iy)
...
plot(FLStocks(stk.om.merr=stk.om.merr, stk.mp=stk.mp)) + theme(legend.position="top") + geom_vline(aes(xintercept=2009))
