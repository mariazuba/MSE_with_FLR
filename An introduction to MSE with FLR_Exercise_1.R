...
stk.om.sr <- fwd(stk, control=ctrl, sr=srsr, sr.residuals = exp(srsr.res), sr.residuals.mult = TRUE)
...
for(i in vy[-length(vy)]){
  ...
  stk.om.sr <- fwd(stk.om.sr, control=ctrl,sr=srsr, sr.residuals = exp(srsr.res), sr.residuals.mult = TRUE) 
}

save(stk.om.sr, file = 'C:/use/Dropbox/FLBEIA Course/Tutorials/02_MSE_with_FLR/stk_exe1.RData')

#---------------------------------------------------------------------------------
# PERFORMANCE STATISTICS
#---------------------------------------------------------------------------------
# Some example performance statstics, but first isolate the projection period
stk.tmp <- window(stk.om.sr,start=iy)
...
plot(FLStocks(stk.om.sr=stk.om.sr, stk.mp=stk.mp)) + theme(legend.position="top") + geom_vline(aes(xintercept=2009))
