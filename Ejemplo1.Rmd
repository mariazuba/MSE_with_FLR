...
# Change the stock-recruitment relationship in the OM for a ricker model.
...
srsr  <- fmle(as.FLSR(stk,  model="segreg"), 
              method="L-BFGS-B", 
              lower=c(1e-6, 1e-6), 
              upper=c(max(rec(stk)) * 3, Inf))

srsr0 <- fmle(as.FLSR(stk0, model="segreg"), 
              method="L-BFGS-B", 
              lower=c(1e-6, 1e-6), 
              upper=c(max(rec(stk)) * 3, Inf)) 
...
srsr.res <- rnorm(it, 
                  ...
                  mean(c(apply(residuals(srsr), 6, sd))))
...
plot(FLStocks(stk.om.sr=stk.om.sr, stk.mp=stk.mp)) + 
