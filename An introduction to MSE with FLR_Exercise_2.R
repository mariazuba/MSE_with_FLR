...
for(i in vy[-length(vy)]){
  ...
  stk.om.m <- fwd(stk.om.m, control=ctrl,sr=srbh, sr.residuals = exp(srbh.res), sr.residuals.mult = TRUE) 
}

save(stk.om.m, file = 'C:/use/Dropbox/FLBEIA Course/Tutorials/02_MSE_with_FLR/stk_exe2.RData')

#---------------------------------------------------------------------------------
# PERFORMANCE STATISTICS
#---------------------------------------------------------------------------------
# Some example performance statstics, but first isolate the projection period
stk.tmp <- window(stk.om.m,start=iy)
...
plot(FLStocks(stk.om.m=stk.om.m, stk.mp=stk.mp)) + theme(legend.position="top") + geom_vline(aes(xintercept=2009))
